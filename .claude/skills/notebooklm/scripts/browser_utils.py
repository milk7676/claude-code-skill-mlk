...
class StealthUtils:
    ...
    def human_type(page: Page, selector: str, text: str, wpm_min: int = 320, wpm_max: int = 480):
        """Type with human-like speed - using fill() for stability"""
        ...
        # Use fill() instead of type() to avoid DOM detachment issues
        # This is more stable for dynamic pages like NotebookLM
        try:
            element.fill(text)
            StealthUtils.random_delay(200, 500)
        except Exception as e:
            # Fallback: try using page.fill() if element.fill() fails
            print(f"⚠️ element.fill() failed, trying page.fill(): {e}")
            try:
                page.fill(selector, text)
                StealthUtils.random_delay(200, 500)
            except Exception as e2:
                print(f"❌ Both fill methods failed: {e2}")
                raise
